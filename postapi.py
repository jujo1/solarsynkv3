...
def PostHAEntity(Serial, UOM, UOMLong, fName, sName, EntityVal):
    ...
    try:
        ...
        if len(parsed_login_json['entity_id']) > 1:
            ...
            print("HA Entity: " + ConsoleColor.WARNING + parsed_login_json['entity_id'] + ":" + ConsoleColor.OKCYAN +
                  f" {EntityVal} {UOM}" + ConsoleColor.ENDC + ConsoleColor.OKGREEN + " OK" + ConsoleColor.ENDC)
...
def ConnectionTest(Serial, UOM, UOMLong, fName, sName, EntityVal):
    ...
    with open('/data/options.json') as options_file:
        ...
        url = f"{httpurl_proto}://" + str(json_settings['Home_Assistant_IP']) + ":" + str(
            json_settings['Home_Assistant_PORT']) + "/api/states/sensor.solarsynkv3_" + Serial + '_' + sName

        # print ("HAToken:" + HAToken)
        # payload = {"attributes": {"unit_of_measurement": f"{UOM}", "friendly_name": f"{fName}"}, "state": f"{EntityVal}"}
        payload = {"attributes": {"device_class": f"{UOMLong}", "state_class": "measurement",
                                  "unit_of_measurement": f"{UOM}", "friendly_name": f"{fName}"}, "state": f"{EntityVal}"}
        # print(payload)
