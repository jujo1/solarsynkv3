"""Settings management for SolarSynkV3."""
...
def DownloadProviderSettings(Token, Serial):
    """
    Download settings from provider and save to file.
    ...
    try:
        ...
        if parsed_inverter_json.get('msg') == "Success":
            print_colored(f"Settings fetch response: {parsed_inverter_json['msg']}",
                          ConsoleColor.OKGREEN)
            with open("svr_settings.json", "w") as file:
                json.dump(parsed_inverter_json, file, indent=4)

            print_colored("Settings download from provider complete",
                          ConsoleColor.OKGREEN)
        else:
            print_colored(f"Settings fetch response: {parsed_inverter_json['msg']}",
                          ConsoleColor.FAIL)
        ...
        print_colored("Error: Request timed out while connecting to provider API.",
                      ConsoleColor.FAIL)
    except requests.exceptions.RequestException as e:
        print_colored(f"Error: Failed to connect to provider API. {e}",
                      ConsoleColor.FAIL)
    except json.JSONDecodeError:
        print_colored("Error: Failed to parse provider API response.",
                      ConsoleColor.FAIL)
...
def GetNewSettingsFromHAEntity(SunSynkToken, Serial):
    ...
    with open('/data/options.json') as options_file:
        ...
        url = f"{httpurl_proto}://" + str(json_settings['Home_Assistant_IP']) + ":" + str(
            json_settings['Home_Assistant_PORT']) + "/api/states/input_text.solarsynkv3_" + Serial + '_settings'
    ...
    try:
        ...
        print(ConsoleColor.FAIL +
              "Error: Request timed out while connecting to Home Assistant API." + ConsoleColor.ENDC)

    except requests.exceptions.RequestException as e:
        print(ConsoleColor.FAIL +
              f"Error: Failed to connect to Home Assistant API. {e}" + ConsoleColor.ENDC)
        print(f"You probably did not create the settings entity. Manually create it for inverter with serial " + ConsoleColor.OKCYAN + Serial + ConsoleColor.ENDC +
              " In the HA GUI in menu [Settings] -> [Devices & Services] -> [Helpers] tab -> [+ CREATE HELPER]. Choose [Text] and name it: " + ConsoleColor.OKCYAN + "solarsynkv3_" + Serial + "_settings" + ConsoleColor.ENDC)

    except json.JSONDecodeError:
        print(ConsoleColor.MAGENTA + "Notice: Invalid or no settings found to post back to sunsynk. This is not a critical error, it just means that the settings you provided in the settings entity (/api/states/input_text.solarsynkv3_" +
              Serial + "_settings) is invalid or blank. If this is intentional just ignore." + ConsoleColor.ENDC)
...
def DetermineSettingCategory(JSON_Search_Key, JSON_Search_Key_val):
    ...
    if JSON_Search_Key in ("absorptionVolt", "battMode", "batteryCap", "batteryEfficiency", "batteryEmptyV", "batteryImpedance", "batteryLowCap", "batteryLowVolt", "batteryMaxCurrentCharge", "batteryMaxCurrentDischarge", "batteryOn", "batteryRestartCap", "batteryRestartVolt", "batteryShutdownCap", "batteryShutdownVolt", "bmsErrStop", "disableFloatCharge", "equChargeCycle", "equChargeTime", "equVoltCharge", "floatVolt", "genChargeOn", "genSignal", "generatorBatteryCurrent", "generatorForcedStart", "generatorStartCap", "generatorStartVolt", "gridSignal", "lithiumMode", "lowNoiseMode", "lowPowerMode", "safetyType", "sdBatteryCurrent", "sdChargeOn", "sdStartCap", "sdStartVol", "sdStartVolt", "signalIslandModeEnable", "sn", "tempco"):
        ...
        print("Battery Setting update request detected for: " +
              JSON_Search_Key + "-->" + JSON_Search_Key_val)
    ...
    if JSON_Search_Key in ("sn", "safetyType", "battMode", "solarSell", "pvMaxLimit", "energyMode", "peakAndVallery", "sysWorkMode", "sellTime1", "sellTime2", "sellTime3", "sellTime4", "sellTime5", "sellTime6", "sellTime1Pac", "sellTime2Pac", "sellTime3Pac", "sellTime4Pac", "sellTime5Pac", "sellTime6Pac", "cap1", "cap2", "cap3", "cap4", "cap5", "cap6", "sellTime1Volt", "sellTime2Volt", "sellTime3Volt", "sellTime4Volt", "sellTime5Volt", "sellTime6Volt", "zeroExportPower", "solarMaxSellPower", "mondayOn", "tuesdayOn", "wednesdayOn", "thursdayOn", "fridayOn", "saturdayOn", "sundayOn", "time1on", "time2on", "time3on", "time4on", "time5on", "time6on", "genTime1on", "genTime2on", "genTime3on", "genTime4on", "genTime5on", "genTime6on"):
        ...
        print("System Mode Setting update request detected for: " +
              JSON_Search_Key + "-->" + JSON_Search_Key_val)
        # Load the JSON data from a file
...
def PostSettingToSunSynk(Token, Serial, SettingsType):
    ...
    if SettingsType == "Battery Settings":
        ...
        try:
            ...
            # Send POST request
            response = requests.post(inverter_url, json=payload,
            ...
            if parsed_json_response.get('msg') == "Success":
                ...
                print(SettingsType + " updated with SunkSynk server response: " +
                      ConsoleColor.OKGREEN + parsed_json_response['msg'] + ConsoleColor.ENDC)
            else:
                print("Update failed with SunkSynk response: " + ConsoleColor.FAIL +
                      parsed_json_response['msg'] + ConsoleColor.ENDC)

        except requests.exceptions.Timeout:
            print(ConsoleColor.FAIL +
                  "Error: Request timed out while connecting to Sunsynk API." + ConsoleColor.ENDC)

        except requests.exceptions.RequestException as e:
            print(ConsoleColor.FAIL +
                  f"Error: Failed to connect to Sunsynk API. {e}" + ConsoleColor.ENDC)

        except json.JSONDecodeError:
            print(ConsoleColor.FAIL +
                  "Error: Failed to parse Sunsynk API response." + ConsoleColor.ENDC)
...
def ResetSettingsEntity(Serial):
    ...
    try:
        ...
        print(ConsoleColor.FAIL +
              "Error: Failed to parse Home Assistant API response." + ConsoleColor.ENDC)
...
def BuildLocalBatterySettings():
    ...
    ArrBattery_settings = ["absorptionVolt", "battMode", "batteryCap", "batteryEfficiency", "batteryEmptyV", "batteryImpedance", "batteryLowCap", "batteryLowVolt", "batteryMaxCurrentCharge", "batteryMaxCurrentDischarge", "batteryOn", "batteryRestartCap", "batteryRestartVolt", "batteryShutdownCap", "batteryShutdownVolt", "bmsErrStop", "disableFloatCharge", "equChargeCycle",
                           "equChargeTime", "equVoltCharge", "floatVolt", "genChargeOn", "genSignal", "generatorBatteryCurrent", "generatorForcedStart", "generatorStartCap", "generatorStartVolt", "gridSignal", "lithiumMode", "lowNoiseMode", "lowPowerMode", "safetyType", "sdBatteryCurrent", "sdChargeOn", "sdStartCap", "sdStartVol", "sdStartVolt", "signalIslandModeEnable", "sn", "tempco"]
...
def BuildLocalSystemModeSettings():
    ...
    ArrsySystemMode_settings = ["sn", "safetyType", "battMode", "solarSell", "pvMaxLimit", "energyMode", "peakAndVallery", "sysWorkMode", "sellTime1", "sellTime2", "sellTime3", "sellTime4", "sellTime5", "sellTime6", "sellTime1Pac", "sellTime2Pac", "sellTime3Pac", "sellTime4Pac", "sellTime5Pac", "sellTime6Pac", "cap1", "cap2", "cap3", "cap4", "cap5", "cap6", "sellTime1Volt",
                                "sellTime2Volt", "sellTime3Volt", "sellTime4Volt", "sellTime5Volt", "sellTime6Volt", "zeroExportPower", "solarMaxSellPower", "mondayOn", "tuesdayOn", "wednesdayOn", "thursdayOn", "fridayOn", "saturdayOn", "sundayOn", "time1on", "time2on", "time3on", "time4on", "time5on", "time6on", "genTime1on", "genTime2on", "genTime3on", "genTime4on", "genTime5on", "genTime6on"]
